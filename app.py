...
async def capture(payload: Capture) -> dict:
    ...
    try:
        async with httpx.AsyncClient(follow_redirects=True, timeout=15, headers={"User-Agent": "Website-Time-Machine/0.1"}) as client:
    ...
    con.commit(); snapshot_id = cur.lastrowid
    con.close()
    ...
    if previous:
        ...
        result["diff"] = {"words_added": len([w for w in new_words if w not in old_set]), "words_removed": len([w for w in old_words if w not in new_set]), "similarity": similarity(previous[4], text), "preview": "ADDED: " + " ".join(added) + "\n\nREMOVED: " + " ".join(removed)}
    return result
